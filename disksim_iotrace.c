...
static ioreq_event * iotrace_ascii_get_ioreq_event (FILE *tracefile, ioreq_event *new)
{
   ...
   if (sscanf(line, "%lf %d %d %d %x\n", &new->time, &new->devno, &new->blkno, &new->bcount, &new->flags) != 5) {
    ...
    isHeadPartial = 0;
    isTailPartial = 0;
    page_num = (new->blkno + new->bcount-1)/8 - (new->blkno)/8 + 1;
	
	ch_sum_page_num += page_num;
	if(ch_sum_page_num  >= 10000){
        ch_sim_time = new->time;
	}
	
    if( page_num == 1 )
        if( (new->blkno % 8 != 0) || ((new->blkno % 8 == 0) && ( new->bcount < 8 )) ){
            isHeadPartial = 1;
            isTailPartial = 1;
        }
        
    if( (page_num > 1) && (new->blkno % 8 != 0) )
        isHeadPartial = 1;

    if( (page_num > 1) && ( (new->blkno + new->bcount) % 8 != 0 ) )
        isTailPartial = 1;
    
    
   new->bcount = page_num * 8;
   new->blkno /= 8;
   new->blkno *= 8;
